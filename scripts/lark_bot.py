...
def _get_role_enclosures(role: str) -> list[str]:
    """从 Settings 配置读取角色对应的宽口围场列表。
    读 config/enclosure_role_override.json 的 wide 字段，
    反转为 role→[M标签] 映射。wide 中不存在的角色返回空列表（不 fallback）。
    仅当配置文件不存在或读取失败时才用 fallback。
    """
    try:
        ...
        pass
    return _ROLE_ENC_FALLBACK.get(role, [])
...
def cmd_followup(args: argparse.Namespace) -> None:
    ...
    if not enc_order:
        print(f"[跳过] {role} 无宽口围场配置，不推送打卡内容")
        return
    ...
    r_total = sum(
        role_enc_map.get(e, {}).get("students", 0) for e in enc_order
    )
    r_checked = sum(
        role_enc_map.get(e, {}).get("checked_in", 0)
        for e in enc_order
    )
    ...
    for enc in enc_order:
        ...
        if ei.get("students", 0) > 0:
            enc_rate_parts.append(f"{enc}: **{ei['rate']:.1%}**")

    overview_elements.append(
        {
            "tag": "markdown",
            "content": (
                f"{_th('total_summary')} **{total_count}**"
                f" {_th('persons')}"
                f"  ({len(team_cc_results)} {_th('teams')})"
                f"  ▸ **{r_rate}**\n"
                f"{_zh('total_summary')} **{total_count}**"
                f" {_zh('persons')}"
    ...
    # 数据行
    for tr in team_cc_results:
        t_rate = tr.get("rate", 0)
        enc_cells = []
        for enc in enc_order:
            te = tr.get("by_enc", {}).get(enc)
            val = f"{te:.1%}" if te and te > 0 else "—"
            enc_cells.append(
                {
                    "tag": "column",
                    "width": "weighted",
    ...
    for idx, tr in enumerate(team_cc_results, start=2):
        ...
        for cc_entry in tr["ccs"]:
            ...
            # CC 名字 + 打卡率 + 图片链接
            cc_header = f"👤 **{cc_entry['cc']}**  ▸ **{rate_pct}**"
            if cc_entry.get("img_url"):
                cc_header += (
                    f"\n📷 [{_th('view_list')}"
                    f" {cc_entry['cc']}]"
                    f"({cc_entry['img_url']})"
            ...
            team_elements.append(
                {"tag": "markdown", "content": cc_header}
            )

            # 围场行（column_set 灰底）+ 学员 ID
