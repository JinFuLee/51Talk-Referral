...
## 第零原则：主题驱动 × 岗位维度（项目底层架构逻辑）

**铁律：页面按"分析主题"组织，不按岗位组织。CC/SS/LP 是维度，不是页面。**

### 原则说明

转介绍业务中 CC/SS/LP 三岗共同服务同一个学员漏斗，区别仅在于负责的围场段不同（0-90→CC / 91-120→SS / 121+→LP）。这个边界是**可配置、会调整的**（`config/enclosure_role_override.json`），而分析主题（围场效率/跟进质量/推荐贡献）是稳定的。

因此：
- **每个页面 = 一个分析主题**（围场分析 / 人员排名 / 跟进质量 / ...）
- **进入页面后，通过 Tab 或筛选切换 CC/SS/LP 视角**
- **禁止为单一岗位建独立页面**（如 `/ss-lp-matrix` 是反模式，应合并到 `/enclosure`）

### 架构规则

| 规则 | 说明 | 违规示例 |
|------|------|---------|
| R1 主题为页 | 一个分析主题 = 一个路由 | ✗ `/cc-matrix` + `/ss-lp-matrix` 拆成两页 |
| R2 岗位为维 | CC/SS/LP 是页面内的 Tab/筛选/列 | ✗ 围场页只展示 CC 不展示 SS/LP |
| R3 边界可配 | 围场×岗位边界从配置读取，不硬编码 | ✗ 代码写死"0-90天=CC" |
| R4 三岗对等 | 有 CC 的分析维度，SS/LP 必须同等提供（数据允许时） | ✗ CC 有排名但 SS/LP 无排名 |
| R5 预留扩展 | 当前只有 CC 数据的维度，SS/LP Tab 预留（显示"暂无数据"） | ✗ 没有 SS 数据就不建 SS Tab |

### 页面结构标准模式

```
/主题路由
  ├── Tab: 全部 | CC | SS | LP        ← 岗位维度切换
  ├── 筛选: 围场段 | 时间范围          ← 业务维度筛选
  └── 内容区: 表格/图表/卡片           ← 按选中岗位展示对应数据
```
...
| 岗位 | 围场过程数据 | 明细数据 | 排名数据 |
|------|------------|---------|---------|
| CC | D2 byCC (641行×25列) | D3 明细 (last_cc_*) | D2 聚合 |
| SS | D2-SS (441行×20列) | D3 明细 (last_ss_*) | D2-SS 聚合 |
| LP | D2-LP (263行×20列) | D3 明细 (last_lp_*) | D2-LP 聚合 |
| 全部 | D2+D2-SS+D2-LP 合并 | D3 全量 | 三源合并排名 |
