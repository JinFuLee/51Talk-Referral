---
name: self-optimize
description: 基于项目实际使用数据，自动分析 Codex 工作模式并调优规则/模板/Skill — 适配 ref-ops-engine 特有的 observe 数据路径和优化目标
...
2. Read `AGENTS.md` 技术债表 — 统计 P1/P2 未解决数量、重复出现类别
...
- **P2 规则层优化**：AGENTS.md 防错认知条目更新建议

### 步骤 4：自主修复（仅限低风险）
**允许直接修复（无需确认）**：
- AGENTS.md 技术债状态更新（已解决标注）
- .gitignore 遗漏项补充
- 命名不一致的文档注释
...
## 与全局 Skill 的关系
- 全局版路径：~/.Codex/skills/self-optimize/SKILL.md（**当前不存在，本文件为项目首版**）
...
- 项目 AGENTS.md 技术债表新增条目时建议触发

### 搜索策略

#### 项目上下文提取（自动）
执行前先 Read 项目 AGENTS.md，提取：
1. **技术栈**：Python FastAPI / Next.js 14 / SQLite / Docker / Recharts / shadcn-ui
2. **业务域**：转介绍运营、数据分析、中泰双语
3. **当前技术债**：从"已知问题与技术债"表提取 P1/P2 条目
4. **已有 Skill 清单**：Glob `.Codex/skills/` 列出已安装的

#### 搜索关键词（项目特化，基于上下文动态生成）
- `Codex {技术栈关键词} skill`（如 `Codex fastapi skill`）
- `Codex {功能需求} skill`（如 `Codex i18n localization skill`）
- `Codex {技术债相关} skill`（如 `Codex sqlite migration tool`）
- `Codex data-analysis visualization skill`
- `Codex excel xlsx processing skill`
...
- 安装到 `.Codex/skills/{name}/SKILL.md`（项目级）
