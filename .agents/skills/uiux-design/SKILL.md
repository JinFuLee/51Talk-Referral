...
when_to_use: UI/UX 任务时自动触发（全局 AGENTS.md 规定）；新增图表组件、页面布局、数据卡片时使用
version: 1.0.0
---
...
- **设计参考文件**：`~/.Codex/contexts/uiux-design-laws.md`（全局设计法则）
...
## 与全局 Skill 的关系
- 全局版路径：~/.Codex/skills/uiux-design/SKILL.md（**当前不存在**）
- 全局设计法则参考：`~/.Codex/contexts/uiux-design-laws.md`（**文件存在，可 Read**）
