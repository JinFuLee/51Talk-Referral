...
- **AGENTS.md 路径**：项目根目录 `AGENTS.md`（里程碑摘要表 + 技术债表）
- **团队架构**：v6.0（见全局 AGENTS.md Agent 架构章节）
- **核心分层**：
...
2. Read `AGENTS.md` 技术债表 — 识别 P1 未解决债务（优先纳入规划）
...
| `meta` | AGENTS.md/roadmap/文档/配置变更 |
| `qa` | 端到端验证（末位执行） |
...
## 与全局 Skill 的关系
- 全局版路径：~/.Codex/skills/orchestrate/SKILL.md（**当前不存在，本文件为项目首版**）
