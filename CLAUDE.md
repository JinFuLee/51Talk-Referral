...
- **围场×岗位负责边界**（由配置驱动，**禁止在代码/文档中硬编码具体围场段**）:
  - **CC 窄口** = 全围场：leads + 过程 + 转化（CC 员工链接覆盖所有学员，不受围场限制）
  - **窄口/宽口各角色的围场分工** → 读取 `projects/referral/config.json` 的 `enclosure_role_assignment`
  - 代码/文档/页面描述中不得出现"CC=M0~M2, SS=M3"等硬编码表述，必须从配置读取
  - 配置变更后，系统行为自动跟随，无需改代码
- **口径×指标归属规则**:
