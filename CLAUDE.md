...
- **围场×岗位负责边界**（可配置，禁止硬编码）:
  - M0~M2（0-90天）已付费学员 → **CC** 负责：全维度全时间转换（full_funnel）
  - M3（91-120天）已付费学员 → **SS** 负责：转介绍leads数 + 过程指标（触达率/打卡率）
  - M4~M5（121-180天）已付费学员 → **LP** 负责：转介绍leads数 + 过程指标（触达率/打卡率）
  - M6~M12+（181天+）已付费学员 → **运营** 负责：转介绍leads数 + 过程指标（触达率/打卡率）
