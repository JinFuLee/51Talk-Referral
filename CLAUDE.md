...
- **围场** = 用户**付费当日**起算天数分段，共 14 段 M0~M12+（每段约30天：M0=0~30, M1=31~60, M2=61~90, M3=91~120, M4=121~150, M5=151~180, M6=181~210, M7=211~240, M8=241~270, M9=271~300, M10=301~330, M11=331~360, M12=361~390, M12+=391+）
...
- **转化归属铁律：CC 是全公司唯一转化端口**
  - 所有付费转化（leads→paid→revenue）都是 CC 完成的，不分围场、不分口径
  - CC 个人业绩 = 全围场全口径的转介绍付费金额，API 强制忽略 enclosure 参数
- **围场×岗位：窄口 vs 宽口分开看**（可配置，禁止硬编码）:
  - **CC 窄口** = 全围场（M0~M12+）：leads + 过程 + 转化（CC 员工链接覆盖所有学员）
  - **CC 宽口** = M0~M2（0-90天）：leads + 过程（学员链接，新付费学员活跃期）
  - **SS 窄口** = M3（91-120天）：leads + 过程指标（触达率/打卡率）
  - **LP 窄口** = M4~M5（121-180天）：leads + 过程指标（触达率/打卡率）
  - **宽口 M3+** = SS/LP/运营各自围场：leads + 过程（宽口 M6+ 归运营）
  - 配置位置: `projects/referral/config.json` 的 `enclosure_role_assignment`
- **口径×指标归属规则**:
  - CC（全漏斗转化）= 注册→预约→出席→付费→金额 + 全部转化率，**不按围场拆分**
...
| 37 | 前端性能 | 47 页面全部 "use client"，无 RSC 收益 | P3 | — | ✅ 已关闭（内部工具可接受，性能满足需求，不做 RSC 迁移）|
