...
const STATUS_ICON = {
  pending: <div className="w-4 h-4 rounded-full border-2 border-hover-token" />,
  sending: <Loader2 className="w-4 h-4 animate-spin text-action-accent" />,
  success: <CheckCircle2 className="w-4 h-4 text-success-token" />,
  error: <XCircle className="w-4 h-4 text-danger-token" />,
};
    ...
    <div className="space-y-3">
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-muted-token">{STATUS_TEXT.progress}</span>
      ...
      <div className="space-y-1.5">
        {items.map((item) => (
          <div
            ...
            className="flex items-center gap-3 py-1.5 px-2 rounded-lg bg-bg-primary"
          >
            {STATUS_ICON[item.status]}
            <div className="flex-1 min-w-0">
              <span className="text-xs font-medium text-primary-token">{item.channel}</span>
              {item.message && item.status === 'error' && (
                <p className="text-xs text-danger-token truncate">{item.message}</p>
              )}
            </div>
            <span
              className={`text-xs ${
