  ...
  return (
    <div className="bg-white border border-[var(--border-default)] rounded-lg shadow-md px-3 py-2 text-xs">
      <p className="font-semibold text-[var(--text-primary)] mb-0.5">
        {item.count === 0 ? '零打卡' : `本月 ${item.count} 次`}
      </p>
      <p className="text-[var(--text-secondary)]">
        学员人数：<span className="font-mono tabular-nums font-semibold">{item.students.toLocaleString()}</span>
      </p>
      <p className="text-[var(--text-secondary)]">
        占比：<span className="font-mono tabular-nums font-semibold">{(item.pct * 100).toFixed(1)}%</span>
      </p>
    </div>
  );
}
...
export function StudentFrequencyChart({ data }: StudentFrequencyChartProps) {
  ...
  const chartData = data.map((item) => ({
    ...item,
    label: item.count === 0 ? '0次' : `${item.count}次`,
  }));
