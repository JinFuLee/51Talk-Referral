...
export function CheckinImpactCard({
  ...
  const strengthColor =
    strength >= 70 ? "text-success" : strength >= 40 ? "text-warning" : "text-slate-500";
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1 bg-primary/10 rounded-lg p-3 text-center">
          ...
          <p className="text-xl font-bold text-primary">{Math.round(checkinRate * 100)}%</p>
        </div>
        ...
        <div className="flex-1 bg-success/10 rounded-lg p-3 text-center">
          ...
          <p className="text-xl font-bold text-success">{referralRate.toFixed(2)}x</p>
        </div>
      </div>
