...
export function NetAttributionSlide({ slideNumber, totalSlides }: SlideProps) {
  const t = useTranslations('NetAttributionSlide');
  ...
  const insight = (() => {
    ...
    return t('insight', {
      ...
      topRevenue: formatRevenue(top.per_capita ?? 0),
      lowChannel: low.channel,
      lowRevenue: formatRevenue(low.per_capita ?? 0),
    });
  })();
    ...
    >
      {isLoading ? (
        ...
        <div className="overflow-auto h-full">
          ...
          <p className="mt-4 px-4 text-xs text-muted-token">{t('footnote')}</p>
        </div>
      )}
    </SlideShell>
  );
