...
const VALID_LANGUAGES = ['zh', 'th'] as const;
...
function validateTimeRange(v: unknown): TimeRange {
  if (typeof v === 'string' && VALID_SIMPLE_TIME_RANGES.includes(v as typeof VALID_SIMPLE_TIME_RANGES[number])) {
...
function validateLanguage(v: unknown): 'zh' | 'th' {
  return VALID_LANGUAGES.includes(v as 'zh' | 'th') ? (v as 'zh' | 'th') : 'zh';
}

function validateRole(v: unknown): 'ops' | 'exec' | 'finance' {
  return VALID_ROLES.includes(v as 'ops' | 'exec' | 'finance') ? (v as 'ops' | 'exec' | 'finance') : 'ops';
}
...
interface ConfigState {
  ...
  setConfig: (config: Partial<Omit<ConfigState, 'setLanguage' | 'setRole' | 'setConfig' | 'setPeriod' | 'setCompareMode' | 'setTimeRange' | 'setTeamFilter' | 'setFocusCC' | 'setSelectionContext' | 'clearSelectionContext'>>) => void;
