...
export function useSummary() {
  ...
  return useSWR(['analysis/summary', period], () => analysisAPI.getSummary(period), REFRESH_30S);
}
...
export function useTeamData() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/team-data', period], () => analysisAPI.getTeamData(period));
}

export function useAnomalies() {
  const period = useConfigStore((s) => s.period);
  return useSWR<AnomalyItem[]>(
    ['analysis/anomalies', period],
...
export function useRiskAlerts() {
  const period = useConfigStore((s) => s.period);
  return useSWR<RiskAlert[]>(
    ['analysis/risk-alerts', period],
...
export function usePrediction() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/prediction', period], () => analysisAPI.getPrediction(period));
}

export function useAttribution() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/attribution', period], () => analysisAPI.getAttribution(period));
}
...
export function useCCRanking(topN = 10) {
  ...
  return useSWR(['analysis/cc-ranking', topN, period, focusCC], () => swrFetcher(key));
}
...
export function useCheckin() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/checkin', period], () => analysisAPI.getCheckin(period));
}
...
export function useFollowup() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/followup', period], () => analysisAPI.getFollowup(period));
}
...
export function useOrders() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/orders', period], () => analysisAPI.getOrders(period));
}
...
export function usePackageMix() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/package-mix', period], () => analysisAPI.getPackageMix(period));
}

export function useTeamPackageMix() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/team-package-mix', period], () => analysisAPI.getTeamPackageMix(period));
}

export function useChannelRevenue() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/channel-revenue', period], () => analysisAPI.getChannelRevenue(period));
}
...
export function useFunnelDetail() {
  ...
  return useSWR(['analysis/funnel-detail', period, focusCC], () => swrFetcher(key));
}
...
export function useOutreachHeatmap() {
  ...
  return useSWR(['analysis/outreach-heatmap', focusCC], () => swrFetcher(key));
}
...
export function useChannelMoM() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/channel-mom', period], () => analysisAPI.getChannelMoM(period));
}
...
export function useTimeInterval() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/time-interval', period], () => analysisAPI.getTimeInterval(period));
}
...
export function useOutreachGap() {
  const period = useConfigStore((s) => s.period);
  return useSWR(['analysis/outreach-gap', period], () => analysisAPI.getOutreachGap(period));
}
...
export function useCompareSummary() {
  ...
  // TODO: 后端 /api/analysis/compare-summary 端点尚未实现，暂时禁用请求防止 404 污染 error-log
  // 恢复方式：将 null 改回 compareMode !== 'off' ? ["analysis/compare-summary", period, compareMode] : null
  return useSWR<ComparisonResponse | null>(null, async () => {
    try {
...
export function useKPISparkline(days = 14) {
  return useSWR(['analysis/kpi-sparkline', days], () => analysisAPI.getKPISparkline(days));
}

export function useLeadsOverview(scope: string = 'total') {
  const period = useConfigStore((s) => s.period);
  return useSWR<LeadsOverviewData>(['analysis/leads-overview', scope, period], () =>
