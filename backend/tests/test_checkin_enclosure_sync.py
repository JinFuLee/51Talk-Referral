...
def test_team_detail_accepts_role_config():
    ...
    r = client.get(
        "/api/checkin/team-detail",
        params={"team": "TH-LP01Team", "role_config": LP_M4_ONLY},
    )
...
def test_team_detail_respects_role_config():
    ...
    r_all = client.get(
        "/api/checkin/team-detail", params={"team": "TH-LP01Team"}
    )
    r_m4 = client.get(
        "/api/checkin/team-detail",
        params={"team": "TH-LP01Team", "role_config": LP_M4_ONLY},
    ...
    if all_count > 0:
        assert m4_count <= all_count, (
            f"role_config 过滤无效: M4={m4_count} > all={all_count}"
        )
...
def test_followup_respects_role_config():
    ...
    r_m4 = client.get(
        "/api/checkin/followup",
        params={"role": "LP", "role_config": LP_M4_ONLY},
    )
    assert r_all.status_code == 200
    assert r_m4.status_code == 200
...
def test_ranking_respects_role_config():
    ...
    r = client.get(
        "/api/checkin/ranking", params={"role_config": LP_M4_ONLY}
    )
    assert r.status_code == 200
