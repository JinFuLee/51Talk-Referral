...
def get_enclosure_health(svc: AnalysisService = Depends(get_service)) -> dict[str, Any]:
    ...
    if isinstance(f7, dict):
        ...
        if isinstance(f7_summary, dict):
            f7_rate = float(
                f7_summary.get("call_coverage")
                or f7_summary.get("followup_rate")
                or f7_summary.get("rate")
                or 0
            )
        else:
    ...
    for enc in enc_order:
        ...
        fu_rate = (
            float(
                (
                    f8_summary.get("effective_coverage")
                    or f8_summary.get("call_coverage")
                )
                if isinstance(f8_summary, dict)
                else 0
            )
            if f8_summary
            else float(
                f8_row.get("followup_rate")
                or f8_row.get("effective_coverage")
                or f8_row.get("call_coverage")
                or 0
            )
        )
        ...
        status = (
            "green"
            if health_score >= 15
            else ("yellow" if health_score >= 8 else "red")
        )
