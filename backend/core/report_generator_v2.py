...
class PyramidReportGenerator:
    ...
    def __init__(self, analysis_result: dict) -> None:
        ...
        self.targets = (
            self.meta.get("targets", {}) or self.data.get("targets", {}) or {}
        )
        ...
        self.time_progress = (
            _pct(
                self.targets.get("时间进度") or _safe_get(self.summary, "time_progress")
            )
            or 0.0
        )
    ...
    def _build_pillars(self) -> list[dict]:
            ...
            pillars.append(
                {
                    ...
                    "expected_revenue_gain_thb": round(
                        impact_usd * EXCHANGE_RATE_THB_USD, 0
                    ),
                    "actions": [
                        "优化打卡激励机制（积分/礼品激励分享）",
                        "增加打卡提醒频次（课后24小时内）",
        ...
        if reg_gap is not None and reg_gap < -0.02:
            ...
            impact_usd = impact_data.get(
                "registration_impact", round(gap_abs * conversion * unit_price, 0)
            )
            pillars.append(
                {
        ...
        if pay_gap is not None and pay_gap < -0.02:
            ...
            unit_price = _pct(self.targets.get("客单价")) or 850.0
            impact_usd = impact_data.get(
                "payment_impact", round(pay_gap_abs * unit_price, 0)
            )
            attend_info = (
                f"当前出席率{attend_rate:.0%}" if attend_rate else "出席率数据待接入"
            )
            pillars.append(
                {
            ...
            pillars.append(
                {
                    ...
                    "expected_revenue_gain_thb": max(
                        0, (rev_target - rev_usd) * EXCHANGE_RATE_THB_USD
                    ),
                    "actions": [
                        "保持当前外呼频次和质量",
                        "关注月末冲刺节奏，避免最后几天断档",
