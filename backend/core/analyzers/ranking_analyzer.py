...
from .utils import _norm_cc, _safe_div, _safe_pct
...
class RankingAnalyzer:
    ...
    def analyze_cc_360(self) -> dict:
        ...
        # F5: 每日外呼
        f5_cc = (
            self.ctx.data.get("ops", {}).get("daily_outreach", {}).get("by_cc", {})
            or {}
        )
        ...
        # A4: 个人 leads
        a4_records = (
            self.ctx.data.get("leads", {})
            .get("leads_achievement_personal", {})
            .get("records", [])
            or []
        )
        a4_cc: dict = {}
        ...
        # E3: 订单（按 seller 聚合收入）
        e3_records = (
            self.ctx.data.get("order", {}).get("order_detail", {}).get("records", [])
            or []
        )
        e3_revenue: dict = {}
        for r in e3_records:
            ...
            if seller:
                e3_revenue[seller] = e3_revenue.get(seller, 0.0) + (
                    r.get("amount_usd") or 0.0
                )

        # F9: 付费用户跟进
        f9_cc = (
            self.ctx.data.get("ops", {})
            .get("monthly_paid_followup", {})
            .get("by_cc", [])
            or []
        )
        f9_cc_norm = {
            _norm_cc(r.get("cc_name", "")): r for r in f9_cc if r.get("cc_name")
        }
        ...
        for norm_name in all_cc_names:
            ...
            team = (
                d1_row.get("team") or f5_row.get("team") or a4_row.get("group") or None
            )
            ...
            checkin_achievement = (
                _safe_div(checkin_rate, checkin_target) if checkin_rate else None
            )
        ...
        for t, vals in teams.items():
            team_averages[t] = {
                "avg_composite": round(statistics.mean(vals["scores"]), 1)
                if vals["scores"]
                else None,
                "avg_checkin": round(statistics.mean(vals["checkin"]), 4)
                if vals["checkin"]
                else None,
                "avg_conversion": round(statistics.mean(vals["conversion"]), 4)
                if vals["conversion"]
                else None,
            }
    ...
    def analyze_cc_ranking(self) -> list:
        ...
        # D5: 打卡参与率 (by_cc = list)
        d5_by_cc_list = (
            self.ctx.data.get("kpi", {})
            .get("checkin_rate_monthly", {})
            .get("by_cc", [])
            or []
        )
        d5_cc: dict = {}
        ...
        # F6: 体验课跟进 (by_cc = dict)
        f6_by_cc_raw = (
            self.ctx.data.get("ops", {}).get("trial_followup", {}).get("by_cc", {})
            or {}
        )
        ...
        f7_by_cc_raw = (
            self.ctx.data.get("ops", {}).get("paid_user_followup", {}).get("by_cc", {})
            or {}
        )
        ...
        a3_by_cc_raw = (
            self.ctx.data.get("leads", {}).get("leads_detail", {}).get("by_cc", {})
            or {}
        )
        ...
        for nm in all_cc:
            ...
            post_paid_followup = (f7.get("monthly_called") or 0) or (
                f9.get("monthly_called") or 0
            )
            ...
            referral_users = (
                a3.get("推荐注册")
                or a3.get("referral_registered")
                or a4.get("referral_users")
                or leads_count
            )
            ...
            checkin_rate = min(
                d1.get("checkin_24h_rate") or d5.get("checkin_rate") or 0.0, 1.0
            )
        ...
        for row in raw_data:
            ...
            composite_score = (
                process_score * 0.25 + result_score * 0.60 + efficiency_score * 0.15
            )
    ...
    def analyze_ss_lp_ranking(self) -> dict:
        ...
        f10_cc = {
            _norm_cc(r.get("cc_name", "")): r for r in f10_list if r.get("cc_name")
        }
        ...
        for nm in all_cc:
            ...
            is_ss = (
                ("-SS" in team_upper)
                or (team_upper.startswith("SS"))
                or ("-EA" in team_upper)
                or (team_upper.startswith("EA"))
                or ("EA" in team_upper and "TEAM" not in team_upper)
            )
            is_lp = (
                ("-LP" in team_upper)
                or (team_upper.startswith("LP"))
                or ("-CM" in team_upper)
                or (team_upper.startswith("CM"))
            )
        ...
        for r in ss_raw:
            r["paid_share"] = (
                _safe_div(r["paid"], ss_total_paid) if ss_total_paid else 0.0
            )
        for r in lp_raw:
            r["paid_share"] = (
                _safe_div(r["paid"], lp_total_paid) if lp_total_paid else 0.0
            )
        ...
        def _build_ranked_list(data_list: list) -> list:
            ...
            active_dims = {
                cat: _redistribute_role(dims) for cat, dims in SS_LP_DIMS.items()
            }
            ...
            for r in data_list:
                ...
                detail = {
                    ...
                    "contact_rate": {
                        "raw": r["contact_rate"],
                        "norm": round(norm_maps_role["contact_rate"][nm], 4),
                    },
                    "checkin_rate": {
                        "raw": r["checkin_rate"],
                        "norm": round(norm_maps_role["checkin_rate"][nm], 4),
                    },
                    "leads": {
                        "raw": r["leads"],
                        "norm": round(norm_maps_role["leads"][nm], 4),
                    },
                    "conversion_rate": {
                        "raw": r["conversion_rate"],
                        "norm": round(norm_maps_role["conversion_rate"][nm], 4),
                    },
                    "paid_share": {
                        "raw": r["paid_share"],
                        "norm": round(norm_maps_role["paid_share"][nm], 4),
                    },
