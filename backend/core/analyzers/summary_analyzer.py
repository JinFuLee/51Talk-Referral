...
class SummaryAnalyzer:
    ...
    def analyze_summary(self) -> dict:
        ...
        def _kpi_daily(
            ...
            """计算日均指标和效率提升需求"""
            daily_avg = (
                round(actual / elapsed_workdays, 2) if elapsed_workdays > 0 else None
            )
        ...
        reg_daily, reg_rem_daily, reg_lift, reg_abs_gap, reg_pace_daily = _kpi_daily(
            reg_actual, reg_target
        )
        paid_daily, paid_rem_daily, paid_lift, paid_abs_gap, paid_pace_daily = (
            _kpi_daily(paid_actual, paid_target)
        )
        rev_daily, rev_rem_daily, rev_lift, rev_abs_gap, rev_pace_daily = _kpi_daily(
            revenue_usd, amount_target
        )
        ...
        return {
            ...
            "checkin_24h": {
                ...
                "achievement": _safe_pct(checkin_rate, checkin_target)
                if checkin_rate
                else None,
            ...
            "overall_status": _status(
                min([g for g in [reg_gap, paid_gap] if g is not None], default=0.0)
            ),
        }
    ...
    def analyze_funnel(self) -> dict:
        ...
        valid_students = int(
            d4_total.get("active_students") or d3_total.get("active_students") or 0
        )

        # 参与率：从 D4/D3 total 获取
        participation_rate = (
            d4_total.get("participation_rate")
            or d3_total.get("participation_rate")
            or 0
        )

        # 触达率：暂用 D4 conversion_rate 近似，待真实外呼数据（>=120s 有效通话）对接
        contact_rate = (
            d4_total.get("conversion_rate") or d3_total.get("conversion_rate") or 0
        )
    ...
    def analyze_student_journey(self) -> dict:
        ...
        ctx = self.ctx
        a3_records = (
            ctx.data.get("leads", {}).get("leads_detail", {}).get("records", []) or []
        )
        e3_records = (
            ctx.data.get("order", {}).get("order_detail", {}).get("records", []) or []
        )
        f6_records = (
            ctx.data.get("ops", {}).get("trial_followup", {}).get("records", []) or []
        )
        f11_records = (
            ctx.data.get("ops", {}).get("pre_class_outreach", {}).get("records", [])
            or []
        )
        ...
        all_sids = {
            str(r.get("学员ID", "")).strip() for r in a3_records if r.get("学员ID")
        }
        ...
        non_outreached_conv = (
            _safe_pct(non_outreached_paid, len(non_outreached))
            if non_outreached
            else None
        )
        lift = (
            _safe_div(outreached_conv, non_outreached_conv)
            if non_outreached_conv
            else None
        )
