...
class OpsAnalyzer:
    ...
    def analyze_trial_followup(self) -> dict:
        ...
        not_called_att = None
        overall_att = f10_referral.get("attendance_rate") or f11_summary.get("overall_attendance_rate")
        if overall_att is not None and called_att is not None and pre_call_rate is not None and 0 <= pre_call_rate < 1:
            try:
                calc_val = (overall_att - called_att * pre_call_rate) / (1.0 - pre_call_rate)
