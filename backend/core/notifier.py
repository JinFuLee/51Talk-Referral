...
class Notifier:
    def __init__(self, config_path: str = None, config: dict = None):
        if config is not None:
            ...
            if config_path is None:
                config_path = str(Path(__file__).resolve().parent.parent.parent / "config" / "notify.json")
    ...
    def send_test(self) -> str:
        ...
        if self.config.get("email", {}).get("enabled"):
            try:
                self._send_email("51Talk 测试通知", "这是一条测试邮件")
                results.append("email: ok")
            except Exception as e:
                results.append(f"email: failed ({e})")
        else:
            results.append("email: disabled")
        if self.config.get("line", {}).get("enabled"):
            try:
                self._send_line("这是一条测试 LINE 通知")
                results.append("line: ok")
            except Exception as e:
                results.append(f"line: failed ({e})")
        else:
            results.append("line: disabled")
        return ", ".join(results)
