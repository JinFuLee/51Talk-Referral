...
class RootCauseEngine:
            ...
            chain.append(
                self._make_why(
                    ...
                    f"参与率仅{part_rate:.1%}（目标{part_target:.1%}），参与学员不足导致带新能力下降",
                    {
                        ...
                        "actual": round(part_rate, 4),
                        "target": round(part_target, 4),
                    },
                )
            )
        else:
        ...
        if outreach < outreach_target:
            chain.append(
                self._make_why(
                    ...
                    {
                        "metric": "outreach_rate",
                        "actual": round(outreach, 4),
                        "target": round(outreach_target, 4),
                    },
                )
            )
        else:
        ...
        if paid_rate < paid_target:
            chain.append(
                self._make_why(
                    ...
                    {
                        "metric": "paid_rate",
                        "actual": round(paid_rate, 4),
                        "target": round(paid_target, 4),
                    },
                )
            )
        else:
        ...
        if attend_rate < attend_target:
            chain.append(
                self._make_why(
                    ...
                    {
                        "metric": "attend_rate",
                        "actual": round(attend_rate, 4),
                        "target": round(attend_target, 4),
                    },
                )
            )
        else:
        ...
        if reserve_rate < reserve_target:
            chain.append(
                self._make_why(
                    ...
                    {
                        "metric": "reserve_rate",
                        "actual": round(reserve_rate, 4),
                        "target": round(reserve_target, 4),
                    },
                )
            )
        else:
        ...
        if paid_gap is not None and paid_gap < self.GAP_YELLOW:
            ...
            chain.append(
                self._make_why(
                    ...
                    {
                        "metric": "unit_price",
                        "actual": round(unit_price, 2),
                        "target": round(unit_target, 2),
                    },
                )
            )
            chain.append(
    ...
    def _get_anomalous_channels(self) -> list[dict]:
        ...
        channels = (
            ...
            else channels_data.get("channels", [])
        )
        ...
        for ch in channels:
            ...
            if rate is not None and rate < threshold:
                anomalous.append(
                    {**ch, "_paid_rate": rate, "_paid_target": paid_target}
                )

        return anomalous

    def _build_channel_registration_chain(
        self, channel_name: str, channel_data: dict
    ) -> list[dict]:
        ...
        reserve_rate = (
            _pct(rates.get("reserve_rate")) if isinstance(rates, dict) else None
        )
        reserve_target = self._get_reserve_target()
        ...
        # Why 3: 出席率
        attend_rate = (
            _pct(rates.get("attend_rate")) if isinstance(rates, dict) else None
        )
        attend_target = self._get_attend_target()
    ...
    def _get_anomalous_enclosures(self) -> list[dict]:
        ...
        segments = (
            ...
            else (enc_data.get("by_enclosure") or enc_data.get("segments", []))
        )
        ...
        # 计算平均触达率
        reach_rates = [
            _pct(e.get("reach_rate"))
            for e in segments
            if _pct(e.get("reach_rate")) is not None
        ]
        ...
        for enc in segments:
            ...
            is_low_reach = (
                reach is not None and avg_reach > 0 and reach < avg_reach * 0.7
            )
        ...
        if roi is not None and roi < 0.8:
            ...
            chain.append(
                self._make_why(
                    ...
                    {
                        "metric": "reach_rate",
                        "actual": round(reach or 0, 4),
                        "target": round(avg_reach, 4),
                    },
                )
            )
            ...
            if reach < reach_target:
                chain.append(
                    self._make_why(
                        ...
                        {
                            "metric": "reach_rate",
                            "actual": round(reach, 4),
                            "target": round(reach_target, 4),
                        },
                    )
                )
            else:
        ...
        if avg_effective is not None and avg_effective < 0.3:
            chain.append(
                self._make_why(
                    ...
                    {
                        "metric": "avg_effective_rate",
                        "actual": round(avg_effective, 4),
                        "target": 0.30,
                    },
                )
            )
    ...
    def _build_checkin_conversion_chain(self) -> list[dict]:
        ...
        chain.append(
            self._make_why(
                1,
                "为什么打卡带新效率低？",
                "，".join(desc) + "，打卡分享未有效转化为注册"
                if desc
                else "打卡带新转化率偏低",
                {
                    "metric": "overall_rate",
                    "actual": round(overall_rate, 4)
                    if overall_rate is not None
                    else None,
                    "target": round(checkin_target, 4),
                }
        ...
        if low_pkg_pct is not None:
            chain.append(
                self._make_why(
                    ...
                    {
                        "metric": "low_tier_ratio",
                        "actual": round(low_pkg_pct, 4),
                        "threshold": 0.50,
                    },
                )
            )
    ...
    def _build_seasonality_chain(self) -> list[dict]:
        ...
        holiday_str = (
            "、".join(holiday_names[:3]) if holiday_names else "泰国节假日/学校假期"
        )
        ...
        if seasonal_factor is not None:
            desc.append(
                f"季节性指数{seasonal_factor:.2f}（1.0为基准，低于0.85为低谷期）"
            )
        ...
        chain.append(
            self._make_why(
                ...
                "、".join(desc) + f"，{low_period}处于季节性低谷"
                if desc
                else f"{low_period}处于历史低谷区间",
                {
                    "metric": "seasonal_factor",
                    "actual": round(seasonal_factor, 3)
                    if seasonal_factor is not None
                    else None,
                    "benchmark": 1.0,
                }
    ...
    def _get_low_roi_channels(self) -> list[dict]:
        ...
        channels = (
            roi_data if isinstance(roi_data, list) else roi_data.get("channels", [])
        )
        if not isinstance(channels, list):
            return []
        ...
        avg_roi = None
        roi_values = [
            _pct(c.get("roi")) for c in channels if _pct(c.get("roi")) is not None
        ]
        if roi_values:
        ...
        for ch in channels:
            ...
            if roi is not None and avg_roi is not None and roi < avg_roi * 0.7:
                is_low = True
            if (
                cost_pct is not None
                and paid_pct is not None
                and paid_pct > 0
                and cost_pct > paid_pct * 2.0
            ):
                is_low = True
            if is_low:
        ...
        if roi is not None and avg_roi is not None:
            ...
            chain.append(
                self._make_why(
                    ...
                    f"成本占比{cost_pct:.1%}，付费贡献{paid_pct:.1%}，成本显著高于产出"
                    if cost_pct and paid_pct
                    else f"{channel_name}渠道成本效益异常",
                    {
                        "metric": "cost_paid_ratio",
                        "cost_pct": cost_pct,
                        "paid_pct": paid_pct,
                    },
                )
    ...
    def _get_cc_efficiency_gap(self) -> Optional[dict]:
        ...
        top_avg = sum(scores_sorted[: max(1, len(scores_sorted) // 4)]) / max(
            1, len(scores_sorted) // 4
        )
        bottom_avg = sum(scores_sorted[-max(1, len(scores_sorted) // 4) :]) / max(
            1, len(scores_sorted) // 4
        )
        ...
        for cc in self.cc_individual:
            ...
            if score is not None and score <= bottom_avg * 1.1:
                ...
                if (
                    ...
                    and process < result * 0.7
                ):
                    low_cc_chars.append("过程指标（外呼/跟进）弱于结果指标")
                elif (
                    process is not None
                    and result is not None
                    and result < process * 0.7
                ):
                    low_cc_chars.append("结果指标（注册/付费）弱于过程投入")

        char_desc = (
            "、".join(set(low_cc_chars[:2]))
            if low_cc_chars
            else "过程执行力弱（外呼量不足/跟进不及时）或转化能力低（话术不达标）"
        )
        chain.append(
            self._make_why(
                2,
    ...
    def _get_action_and_impact(
        self, metric_key: str, gap_pct: float
    ) -> tuple[str, float]:
        """返回 (行动建议, 预估损失USD)"""
        if metric_key == "registration":
            ...
            return (
                "增加CC外呼频次，优先覆盖新围场0-30天学员；优化打卡激励机制提升分享裂变",
                impact,
            )
    ...
    def analyze(self) -> dict:
        ...
        for m in metric_config:
            ...
            root_cause = (
                why_chain[-1].get("answer", "根因待深入分析")
                if why_chain
                else "数据不足，无法完成完整因果链"
            )
        ...
        for ch in self._get_anomalous_channels():
            ...
            impact = round(
                gap_pts * self._calc_impact_per_point("channel_paid_rate"), 0
            )
            total_loss += impact
        ...
        if self._is_productivity_anomalous():
            ...
            calls_target = _pct(self.targets.get("outreach_calls_per_day", 30)) or 30
            gap_pts = max(
                0.0, (calls_target - (avg_calls or calls_target)) / calls_target
            )
            impact = round(gap_pts * self._calc_impact_per_point("productivity"), 0)
            total_loss += impact
            root_cause = (
                chain[-1].get("answer", "培训和工具效率待提升")
                if chain
                else "CC人效待优化"
            )
            ...
            analyses.append(
                {
                    "trigger": f"CC日均外呼{avg_calls:.1f}次，低于目标{calls_target:.0f}次"
                    if avg_calls is not None
                    else "CC人效指标异常",
        ...
        if self._is_checkin_anomalous():
            ...
            impact = round(
                gap_pts * self._calc_impact_per_point("checkin_conversion"), 0
            )
            total_loss += impact
            root_cause = (
                chain[-1].get("answer", "激励体系待优化") if chain else "打卡转化待改善"
            )
        ...
        for ch_data in self._get_low_roi_channels():
            ...
            total_loss += impact
            root_cause = (
                chain[-1].get("answer", "渠道ROI复盘机制待建立")
                if chain
                else "渠道资源错配"
            )
        ...
        if cc_gap_info is not None:
            ...
            total_loss += impact
            root_cause = (
                chain[-1].get("answer", "CC能力提升体系待建立")
                if chain
                else "团队效率分化"
            )
