...
class EnclosureLPLoader(BaseLoader):
    """加载 D2-LP 围场过程数据（byLP 维度），过滤有效围场"""

    SHEET_NAME = "转介绍中台检测_围场过程数据_byLP"
    FILE_PATTERN = "*围场过程数据*byLP*.xlsx"

    def load(self) -> pd.DataFrame:
        file_path = self._find_file()
        self.last_loaded_file = file_path
        if file_path is None:
            logger.warning("D2-LP 围场过程数据文件未找到")
            return pd.DataFrame()

        df = self._read_xlsx_pandas(file_path, sheet_name=self.SHEET_NAME)
        if df.empty:
            logger.warning(f"D2-LP 围场过程数据为空: {file_path.name}")
            return df
        ...
        if "是否有效" in df.columns:
            ...
            logger.info(f"D2-LP 有效围场过滤后: {len(df)} 行")

        logger.info(f"D2-LP 加载成功: {len(df)} 行, {file_path.name}")
        return df
